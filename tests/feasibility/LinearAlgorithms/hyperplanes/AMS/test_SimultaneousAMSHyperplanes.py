...
def test_SimultaneousAMSHyperplane_step_full(get_full_variables):
    ...
    x_n = alg.step(x_1)
    assert np.all(np.abs(x_n - np.array([5 / 3, 5 / 3])) < 1e-10)
    assert np.array_equal(x_n, x_1)

    # check that project gives the same result as step
    x_1 = np.array([2.0, 2.0])
    x_proj = alg.project(x_1)
    assert np.all(np.abs(x_proj - np.array([5 / 3, 5 / 3])) < 1e-10)
    assert np.array_equal(x_proj, x_1)
    assert np.array_equal(x_proj, x_n)

    x_n = alg.step(x_2)
    assert np.all(np.abs(x_n - np.array([4 / 3, 2])) < 1e-10)
    assert np.array_equal(x_n, x_2)

    x_n = alg.step(x_3)
    assert np.all(np.abs(x_n - np.array([5 / 6, 11 / 6])) < 1e-10)
    assert np.array_equal(x_n, x_3)

    x_n = alg.step(x_4)
    assert np.all(np.abs(x_n - np.array([-1 / 3, 1])) < 1e-10)
    assert np.array_equal(x_n, x_4)

    x_n = alg.step(x_5)
    assert np.all(np.abs(x_n - np.array([-7 / 6, -1 / 6])) < 1e-10)
...
def test_SimultaneousAMSHyperplane_algorithmic_relaxation_step(get_sparse_variables):
    ...
    x_n = alg.step(x_1)
    assert np.all(np.abs(x_n - np.array([4 / 3, 4 / 3])) < 1e-10)
    assert np.array_equal(x_n, x_1)

    # check that project gives the same result as step
    x_1 = np.array([2.0, 2.0])
    x_proj = alg.project(x_1)
    assert np.all(np.abs(x_proj - np.array([4 / 3, 4 / 3])) < 1e-10)
    assert np.array_equal(x_proj, x_1)
    assert np.array_equal(x_proj, x_n)

    x_n = alg.step(x_2)
    assert np.all(np.abs(x_n - np.array([5 / 3, 1])) < 1e-10)
    assert np.array_equal(x_n, x_2)

    x_n = alg.step(x_3)
    assert np.all(np.abs(x_n - np.array([10 / 6, 4 / 6])) < 1e-10)
    assert np.array_equal(x_n, x_3)

    x_n = alg.step(x_4)
    assert np.all(np.abs(x_n - np.array([4 / 3, 0])) < 1e-10)
    assert np.array_equal(x_n, x_4)

    x_n = alg.step(x_5)
    assert np.all(np.abs(x_n - np.array([4 / 6, -2 / 6])) < 1e-10)
    assert np.array_equal(x_n, x_5)
...
def test_SimultaneousAMSHyperplane_proximity(get_full_variables):
    ...
    assert np.abs(alg.proximity(x_3) - 7.25 / 3) < 1e-10
    assert np.abs(alg.proximity(x_4) - 14 / 3) < 1e-10
    assert np.abs(alg.proximity(x_5) - 19.25 / 3) < 1e-10
